...
+ (LPSessionController *)sharedInstance {
  ...
  dispatch_once(&oncePredicate, ^{
    _sharedInstance = [[LPSessionController alloc]
        initWithPersistenceType:LPSessionPersistenceTypeUserDefaults];
  });
...
- (void)setServiceWithServiceType:(LPSessionServiceType)serviceType {
  ...
  _service = [LPSessionServiceBuilder serviceWithType:serviceType
                                             delegate:self];
}
...
- (void)setPersistenceWithPersistenceType:
  ...
  _persistence =
      [LPSessionPersistenceBuilder persistenceWithType:persistenceType];
}
