...
- (void)viewDidLoad {
  [super viewDidLoad];
	...
	[[UINavigationBar appearance] setBarTintColor:[UIColor colorWithRed:0.97 green:0.97 blue:0.97 alpha:1]];

	// _photos = [LPPhotoFetcher getLatestPhotosWithLimit:(NSInteger *)[@20 integerValue]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    LPCustomTableViewCell *cell = (LPCustomTableViewCell *)[tableView dequeueReusableCellWithIdentifier:kSimpleTableIdentifier];

  if (cell == nil) {
      NSArray *nib = [[NSBundle mainBundle] loadNibNamed:kSimpleTableIdentifier owner:self options:nil];
	...
	[cell.bgImageView setImageWithURL:[NSURL URLWithString:photo.photoURI] placeholderImage:[UIImage imageNamed:@"placeholder.jpg"] usingActivityIndicatorStyle:UIActivityIndicatorViewStyleGray];
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
  if ([segue.identifier isEqualToString:@"ItemDetail"]) {
    LPItemDetailViewController *itemDetailViewController = segue.destinationViewController;
