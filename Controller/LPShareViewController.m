...
- (void)setPreviewImage:(UIImage *)image {
	_image = [UIImage imageWithCIImage:image.CIImage scale:1.0 orientation:UIImageOrientationRight];
}
		...
		if (shareButton && [shareButton isKindOfClass:[UIButton class]]) {
			...
			if (button.selected) {
				if (key == kTwitterButton) {
					...
					// LPShare *share = [[LPShare alloc] initWithFramework:LPShareFrameworkTwitter];
					// [share shareContent:@"some text"];
